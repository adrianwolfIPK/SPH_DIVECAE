...
from fisherman.types import Cube
...
file =  "Luftduese_1.stl"
nozzle_name = f"inlet{file}"
path_nozzle = "static/Luftduese_1.stl"
path_nozzle_inlet = "static/inlet_ENG-0468024241235.stl"

nozzle_diamater = 0.00059
d_particle = round(nozzle_diamater/10, 8)

# specify parameters of simulation
inlet_velocity = 5
speed_of_sound = get_sos(inlet_velocity, d_particle) # capped to 1000m/s
end_time = 2
output_number = 151
...
if sim_found == False:
    ...
    # # add nozzle
    # nozzle = simulation.add_wall_boundary(name=file, filepath=path_nozzle)
    # nozzle.scale([0.001, 0.001, 0.001])
    # # -35.98, -35.59, -6.71 for ~ 0,0,0
    # # -35.5, -35.385, -6 for 3/4 top, right
    # nozzle.translate([-35.5, -35.385, -6])
    # nozzle.rotate([0, 45, 90])

   
    # add nozzle inlet
    # 36, -1.7, 0.80 inlets nozzle oririgin
    ...
    nozzle_inlet = simulation.add_open_boundary(name=nozzle_name, filepath=path_nozzle_inlet, open_boundary=boundary_condition_inlet)
    nozzle_inlet.scale([0.001, 0.001, 0.001])
    nozzle_inlet.translate([-35.467, 1.96471, -0.0807])
    nozzle_inlet.rotate([0, 225, 90])
...
# new method?
# fluid_params = fluids["air_5bar_15C"]
# air = simulation.use_material(
